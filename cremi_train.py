...
class Train(object):
    ...
    def placeholders(self):
        ...
        vali_image_placeholder and vali_label_placeholder are for validation images and labels
        lr_placeholder is for learning rate.
        '''
    ...
    def train(self):
        ...
        # For the first step, we are loading all training images and validation images into memory
        dataset = load()
        all_data, all_labels = prepare_train_data(dataset, padding_size=FLAGS.padding_size)
        vali_data, vali_labels = read_validation_data(dataset)


        # Build graph for train and validation
        ...
        # This is for tensorboard useage
    ...
    def generate_vali_batch(self, vali_data, vali_label, vali_batch_size):
        ...
        '''
        offset = np.random.choice(len(vali_data) - vali_batch_size, 1)[0]
    ...
    def full_validation(self, loss, top1_error, session, vali_data, vali_labels, batch_data,
                        batch_label):
        '''
        Runs validation on all valdiation images
        :param loss: tensor with shape [1]
        :param top1_error: tensor with shape [1]
        ...
        :return: float, float
        '''
        num_batches = len(vali_data) // FLAGS.validation_batch_size
        order = np.random.choice(len(vali_data), num_batches * FLAGS.validation_batch_size)
